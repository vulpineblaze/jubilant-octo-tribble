...
EntityHud = ig.Entity.extend({
    ...
    update: function(){
        ig.log(ig.game.turnTimer);
        this.timer= -(ig.game.turnDuration - (ig.game.levelTimer.delta() - ig.game.turnTimer ));
        this.pos.x=ig.game.screen.x;
        this.pos.y=ig.game.screen.y;
    ...
    draw: function(){
        ...
        ig.game.font.draw( 
            'Bruhs: '+ ig.game.stats.bruh  
            +'  |  Foods: '+ ig.game.stats.food  
            +'  |  Woods: '+ ig.game.stats.wood  
            +'  |  Ores: '+ ig.game.stats.ore  
            +'  |  Time: '+   (-this.timer.toFixed(1))
        ...
        if(ig.game.clack.has){
            ig.game.font.draw("WAITING for bruhs to travels",
                ig.system.width/2,
                ig.system.height/3,
                ig.Font.ALIGN.CENTER );
        }
